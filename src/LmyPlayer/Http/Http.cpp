...
const WCHAR* CHttp::GetHeaderDataW()
{
    if (!m_IsRequest)
    {
        GetHttpRequestHeader();
    ...
    DWORD cbSize = 4*1024*1024;
    m_tmp_header = (LPWSTR)AllocMemory(cbSize*sizeof(WCHAR));
#ifdef USE_WINHTTP
    if ( QueryInfo(m_hRequest, WINHTTP_QUERY_RAW_HEADERS_CRLF, m_tmp_header, &cbSize) )
#else // USE_WINHTTP
    if ( QueryInfo(m_hRequest, HTTP_QUERY_RAW_HEADERS_CRLF, m_tmp_header, &cbSize) )
#endif // USE_WINHTTP
...
const char* CHttp::GetHeaderDataA()
{
    LPCWSTR lpData = GetHeaderDataW();
    if (lpData != NULL)
    {
        if (m_tmp_string)
        {
